...
public class PanelAdmin {
    ...
    public static Pane crear(ServicioProducto servicioProducto, ServicioCategoria servicioCategoria, ServicioVenta servicioVenta, pe.edu.upeu.sysventas.servicio.ServicioUsuario servicioUsuario, Runnable onProductAdded) {
        ...
        tv.getColumns().addAll(cId, cNom, cCat, cStock, cPrecio);
        ...
        btnAddProd.setOnAction(e -> {
            try {
                ...
                // Para mantener compatibilidad si Producto aún espera talla/color en su constructor,
                // se envían "" como valores vacíos (ya que para una cafetería no aplica).
                Producto p = new Producto(
                        idField.getText().isEmpty() ? null : idField.getText(),
                        nom.getText().trim(),
                        cat.getText().trim(),
                        "", // talla eliminado -> vacío
                        "", // color eliminado -> vacío
                        Double.parseDouble(precio.getText()),
                        Integer.parseInt(stock.getText()),
                        ""
                );
                if (p.getId() == null || p.getId().isEmpty()) servicioProducto.crear(p); else servicioProducto.actualizar(p);
                idField.clear(); nom.clear(); cat.clear(); precio.clear(); stock.clear();
                items.setAll(servicioProducto.todos());
        ...
        root.getChildren().addAll(titulo,
                ...
                new HBox(6, nom, cat, precio, stock),
                btnAddProd,
                new Separator(),
                tv,
                new HBox(8, btnDelete)
        );
